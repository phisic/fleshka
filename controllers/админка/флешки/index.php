...
class zzRoot extends zzSite {
    ...
    function doInit(){
        $this->ZZ('Admin_Unit.editor')->each('sqlClass', array(
            new Market_Goods_DataContainer( 
                zzNew('Market_Db_Flashrf_DescriptionSize') 
            )
        ));
        ...
        $this->ZZ('Admin_Unit.specialoffer')->each('sqlClass', array(
            new Admin_DataContainer( 
                zzNew('Market_Db_Flashrf_Specialoffer') 
            )
        ));
